...
describe("App", () => {
  ...
  describe("api/topics", () => {
    ...
    describe("POST /api/topics", () => {
      const newTopic = {
        slug: "formula 1",
        description: "the highest class of international racing for open-wheel single-seater formula racing cars",
      };
      test("status:201, responds with the new topic added to the database", () => {
        return request(app)
          .post("/api/topics")
          .send(newTopic)
          .expect(201)
      ...
      test("status 400: empty body sent", () => {
        return request(app)
          .post("/api/topics")
          .send({})
          .expect(400)
          .then(({ body }) => {
            const { msg } = body;
            expect(msg).toBe("Body sent is not valid");
          });
      });
      ...
      test("status 201: ignore extra properties", () => {
        return request(app)
          .post("/api/topics")
          .send({
            ...newTopic,
      ...
      test("status 400: topic already exists", () => {
        return request(app)
          .post("/api/topics")
          .send({
            slug: "cats",
            description: "fluffy animals",
          })
          .expect(400)
          .then(({ body }) => {
            const { msg } = body;
  ...
  describe("api/articles", () => {
    ...
    describe('POST /api/articles', () => {
      test("status 201: responds with 201 for successful request with a new article", () => {
        const articleTest = {
        ...
        test('400: POST responds with status 400 for post request with incorrect keys', () => {
          const testArticle = {
            title: "Video of cat singing",
            body: "Here's a great vid of a cat belting out some tunes",
            votez: 0,
    ...
    describe ("DELETE /api/articles/:article_id", () => {
      test("status 204: responds with empty response", () => {
        return request(app)
      ...
      test("status 404: article doesn't exist", () => {
        return request(app)
          ...
          .then(({ body }) => {
            const { msg } = body;
            expect(msg).toBe("The following article ID provided does not exist: 0");
          });
      });
  ...
  describe("api/comments", () => {
    ...
    describe("PATCH /api/comments/:comment_id", ()=>{
      test("Status 200: responds with the updated comment", ()=>{
        ...
        .expect(200)
        .then((response) => {
          expect(response.body.comment.votes).toEqual(15)
        })  
      })
      test("Status 404: responds with error message when comment doesn't exist", ()=>{
        return request(app)
        .patch("/api/comments/35252")
        .send({inc_votes: 1})
        .expect(404)
        .then((response) => {
          expect(response.body.msg).toBe("Path not found")
